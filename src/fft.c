...
void fft_to_spectra(fits* fit, fftw_complex *frequency_repr, double *as, double *ps){
	unsigned int i;
...
void change_symmetry(fits* fit, unsigned int i, unsigned int j, unsigned int *x, unsigned int *y) {
	int width = fit->rx;
	int height = fit->ry;
...
double normalisation_spectra(fits* fit, double *modulus, double* phase, WORD *abuf, WORD *pbuf, int type_order) {
	unsigned int i, j;
	int width = fit->rx;
	int height = fit->ry;
	double max_m = 0.0;
	...
	for(j=0; j<height; j++){
		for(i=0; i<width; i++){
			...
			pbuf[j*width+i] = round_to_WORD(((phase[y*width+x] + M_PI) * USHRT_MAX_DOUBLE / (2*M_PI)));
			abuf[j*width+i] = round_to_WORD((modulus[y*width+x] * USHRT_MAX_DOUBLE / max_m));
		}
	}
...
void FFTD(fits *fit, fits *x, fits *y, int type_order, int layer){
	...
	assert(nbdata);
	fftw_complex *spatial_repr		= (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * nbdata);
...
#pragma omp parallel for num_threads(com.max_thread) private(i) schedule(dynamic,1)
	...
	x->dft_norm[layer] = normalisation_spectra(fit, modulus, phase, xbuf, ybuf, type_order);
	if (type_order == 0) strcpy(x->dft_ord, "CENTERED");
	else strcpy(x->dft_ord, "REGULAR");
...
void FFTI(fits *fit, fits *xfit, fits *yfit, int type_order, int layer){
	...
	for(j=0;j<height;j++){
		for(i=0;i<width;i++){
			...
			if (type_order == 0)
				change_symmetry(fit, i, j, &x, &y);		
			modulus[j*width+i]	= (double)xbuf[y*width+x] * (xfit->dft_norm[layer]);
...
gpointer fourier_transform(gpointer p){
	...
	case 'D':
		...
		/* ******************************************* */
		new_fit_image(&wfit[1], args->fit->rx, args->fit->ry, args->fit->naxes[2]);
		new_fit_image(&wfit[2], args->fit->rx, args->fit->ry, args->fit->naxes[2]);
		for (chan = 0; chan < args->fit->naxes[2]; chan++)
	...
	case 'I':
		...
		new_fit_image(args->fit, wfit[0].dft_rx, wfit[0].dft_ry, wfit[0].naxes[2]);
		for (chan = 0; chan < args->fit->naxes[2]; chan++) {	
			from[chan] = wfit[2].pdata[chan];
			to[chan]	= args->fit->pdata[chan];
			memcpy(to[chan], from[chan], wfit[0].dft_rx * wfit[0].dft_ry * sizeof(WORD));
		}
	}
	clearfits(&wfit[0]);
	clearfits(&wfit[1]);
