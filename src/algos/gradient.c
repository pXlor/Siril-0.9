...
static gsl_matrix *grad_extract_1color(gsl_vector *MatR, Im2sub *Info) {
	size_t i, j, k, n;
	...
	GtkComboBox *combo_order = GTK_COMBO_BOX(
			gtk_builder_get_object(builder, "combo_polyorder"));
	...
	for (row = 0; row < box_per_col; row++) {
		...
		for (col = 0; col < box_per_row; col++) {
			...
			com.grad[inc].centre.y = height - gsl_vector_get(Vec_row, row)
					+ midbox;
...
static int grad_extract_background(fits *imgfit, fits *bkgfit, int layer) {
	...
	Info->box = (size_t) gtk_spin_button_get_value(
			GTK_SPIN_BUTTON(lookup_widget("spinbutton_bkg_sizebox"))) * 2;
	int interval = (size_t) gtk_spin_button_get_value(
			GTK_SPIN_BUTTON(lookup_widget("spinbutton_bkg_Box_sep")));
	double tolerance = (double) gtk_spin_button_get_value(
			GTK_SPIN_BUTTON(lookup_widget("spinbutton_bkg_tolerance")));
	double deviation = (double) gtk_spin_button_get_value(
	...
	Info->box_per_col = (size_t) ((double) Info->row
			/ ((double) Info->box + interval - 1));
	...
	if (!(Bkg = grad_extract_1color(MatR, Info))) {
...
void grad_background_extraction(fits *fit) {
	int i;
	...
	for (i = 0; i < com.uniq->nb_layers; i++)
		if (grad_extract_background(&gfit, fit, i)) {
