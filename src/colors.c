...
gpointer extract_channels(gpointer p) {
	struct extract_channels_data *args = (struct extract_channels_data *)p;
	WORD *buf[3] = {args->fit->pdata[RLAYER], args->fit->pdata[GLAYER], args->fit->pdata[BLAYER]};
	int i;
	struct timeval t_start, t_end;
	args->process = TRUE;
	
	if (args->fit->naxes[2] != 3) {
		siril_log_message("Siril cannot axtract layers. Make sure your image is in RGB mode.\n");
		args->process = FALSE;
...
#pragma omp parallel for num_threads(com.max_thread) private(i) schedule(static)
		for (i = 0; i < args->fit->rx*args->fit->ry; i++) {
			...
			buf[GLAYER][i] = round_to_WORD (((a + 128) / 255.) * USHRT_MAX_DOUBLE);	// -128 < a < 127
			buf[BLAYER][i] = round_to_WORD (((b + 128) / 255.) * USHRT_MAX_DOUBLE);	// -128 < b < 127
		}			
...
void on_button_bkg_selection_clicked(GtkButton *button, gpointer user_data){
	...
	if ((!com.selection.h) || (!com.selection.w)) {
		show_dialog("Make a selection of the background area before", "Warning", "gtk-dialog-warning");
		return;
	}
	
	if (!selection_black_value[0]) {
		selection_black_value[0] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_bkg_x"));
		selection_black_value[1] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_bkg_y"));
		selection_black_value[2] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_bkg_w"));
		selection_black_value[3] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_bkg_h"));
	}
...
void initialize_calibration_interface() {
	static GtkAdjustment *selection_black_adjustment[4] = {NULL, NULL, NULL, NULL};
	static GtkAdjustment *selection_white_adjustment[4] = {NULL, NULL, NULL, NULL};

	if (!selection_black_adjustment[0]) {
		selection_black_adjustment[0] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_bkg_x"));
		selection_black_adjustment[1] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_bkg_y"));
		selection_black_adjustment[2] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_bkg_w"));
		selection_black_adjustment[3] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_bkg_h"));
	}
	if (!selection_white_adjustment[0]) {
		selection_white_adjustment[0] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_white_x"));
		selection_white_adjustment[1] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_white_y"));
		selection_white_adjustment[2] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_white_w"));
		selection_white_adjustment[3] = GTK_ADJUSTMENT(gtk_builder_get_object(builder,"adjustment_white_h"));
	}
...
void background_neutralize (fits* fit, rectangle black_selection) {
	...
	for (layer = 0; layer<com.uniq->nb_layers; layer++) {
		...
		for (i=0; i<fit->rx*fit->ry; i++) {
			if (buf[i]<offset) buf[i]=0;
			else buf[i] = (buf[i]-offset >= USHRT_MAX ? USHRT_MAX : buf[i]-offset);
...
void on_button_white_selection_clicked(GtkButton *button, gpointer user_data){
	...
	if (!selection_white_value[0]) {
		selection_white_value[0] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_white_x"));
		selection_white_value[1] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_white_y"));
		selection_white_value[2] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_white_w"));
		selection_white_value[3] = GTK_SPIN_BUTTON(gtk_builder_get_object(builder,"spin_white_h"));
	}
	
	if ((!com.selection.h) || (!com.selection.w)) {
		show_dialog("Make a selection of the white area before", "Warning", "gtk-dialog-warning");
		return;
	}	
...
void get_coeff_for_wb(fits *fit, rectangle selection, double coef[]) {
	...
	for (chan = 0; chan < fit->naxes[2]; chan++) {
		...
		int k=1;
		for (i = -20; i < 20; i++){
			if ((bin + i > USHRT_MAX) || ((int)bin +i < 0))  continue;
...
void white_balance(fits *fit, gboolean is_manual, rectangle white_selection, rectangle black_selection) {
	int chan, nb_chan;
	double coef[3];
...
void on_calibration_apply_button_clicked(GtkButton *button, gpointer user_data){
	...
	if (!black_selection.w || !black_selection.h) {
		show_dialog("Make a selection of the black area before", "Warning", "gtk-dialog-warning");
		return;
	}
...
void on_checkbutton_manual_calibration_toggled(GtkToggleButton *togglebutton, gpointer user_data) {
